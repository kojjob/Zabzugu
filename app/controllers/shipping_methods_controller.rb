class ShippingMethodsController < ApplicationController
  ...
  def destroy
    ...
    respond_to do |format|
      format.html { redirect_to shipping_methods_path, status: :see_other, notice: "Shipping method was successfully destroyed." }
  ...
  private
    ...
    def set_shipping_method
      @shipping_method = ShippingMethod.find(params.expect(:id))
    end

    # Only allow a list of trusted parameters through.
    def shipping_method_params
      params.expect(shipping_method: [ :name, :delivery_estimate, :provider ])
    end
end
